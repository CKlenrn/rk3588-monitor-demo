...
plane_info()
{
    expected_plane_size=$input_size
    if grep -q '^RGA landscape path enabled:' "$client_log" 2>/dev/null; then
        expected_plane_size=1080x1920
    fi

    awk -v expected="$expected_plane_size" -v expected_format="$input_format" '
        function finish() {
...
validate_trace()
{
    awk -v mode="$mode" '
        ...
        /^RGA landscape path enabled:/ { rga_enabled = 1 }
        $1 == "SYNC" {
            ...
            if (event == "shutdown") {
                if (shutdown_seen)
                    reject("duplicate shutdown marker", NR)
                shutdown_seen = 1
                next
            }
            if (shutdown_seen && event != "rga-wait" && event != "rga-done") {
                reject("event after shutdown: " event, NR)
                next
            }
            ...
            if (mode == "baseline") {
                if (event == "dqbuf") {
                    if (state[key] != 0 || fence != -1)
                        ...
                        state[key] = 1
                } else if (event == "rga-submit") {
                    if (!rga_enabled || state[key] != 1 ||
                        rga_started[key] || fence != -1)
                        reject("invalid implicit RGA submit", key)
                    else
                        rga_started[key] = 1
                } else if (event == "rga-release") {
                    if (!rga_enabled || state[key] != 1 || !rga_started[key] ||
                        rga[key] || fence + 0 < 0)
                        reject("invalid implicit RGA release", key)
                    else {
                        rga[key] = 1
                        rga_acquire[key] = fence
                    }
                } else if (event == "rga-wait") {
                    if (!rga_enabled || state[key] != 1 || !rga[key] ||
                        rga_waiting[key] || fence + 0 < 0)
                        reject("invalid implicit RGA wait", key)
                    else {
                        rga_waiting[key] = 1
                        rga_completion[key] = fence
                    }
                } else if (event == "rga-done") {
                    if (!rga_enabled || state[key] != 1 || !rga_waiting[key] ||
                        rga_done[key] ||
                        fence != rga_completion[key])
                        reject("invalid implicit RGA completion", key)
                    else {
                        rga_done[key] = 1
                        rga_complete++
                    }
                    ...
                    if (state[key] != 1 ||
                        (rga_enabled && (!rga[key] || !rga_done[key])))
                ...
                if (event == "dqbuf") {
                    ...
                    if (!rga_enabled || state[key] != 1 || rga_started[key] ||
                        fence != acquire[key])
                        reject("invalid explicit RGA submit", key)
                    else
                        rga_started[key] = 1
                } else if (event == "rga-release") {
                    if (!rga_enabled || state[key] != 1 || !rga_started[key] ||
                        rga[key] || fence + 0 < 0)
                        reject("invalid explicit RGA release", key)
                    else {
                        rga[key] = 1
                        rga_acquire[key] = fence
                    }
                    ...
                    if (state[key] != 1 ||
                        (rga_enabled && !rga[key]) ||
                        (rga[key] && fence != rga_acquire[key]) ||
                        (!rga[key] && fence != acquire[key]))
                    ...
                    if (!rga_enabled ||
                        ((state[key] != 5 && state[key] != 6) &&
                         !(shutdown_seen &&
                           (state[key] == 3 || state[key] == 4))) ||
                        !rga[key] || rga_waiting[key] || fence + 0 < 0)
                        reject("invalid explicit RGA wait", key)
                    else {
                        rga_waiting[key] = 1
                        rga_completion[key] = fence
                    }
                    ...
                    if (!rga_enabled ||
                        ((state[key] != 5 && state[key] != 6) &&
                         !(shutdown_seen &&
                           (state[key] == 3 || state[key] == 4))) ||
                        !rga_waiting[key] || rga_done[key] ||
                        fence != rga_completion[key])
                        reject("invalid explicit RGA completion", key)
                    else {
                        rga_done[key] = 1
                        rga_complete++
                    }
                } else if (event == "qbuf") {
                    if (rga_enabled && !rga_done[key]) {
                        reject("qbuf before RGA completion", key)
                    } else if (state[key] == 5) {
                        state[key] = 7
                        fenced++
                    } else if (state[key] == 6) {
        ...
        END {
            ...
            if (rga_enabled && !shutdown_seen)
                reject("missing shutdown marker", "all")
            if (rga_enabled && rga_complete < 30)
                reject("fewer than 30 complete RGA chains", "all")
            if (mode == "baseline") {
                if (complete < 30)
            ...
            print "rga_complete=" rga_complete + 0
...
if grep -Eq 'HDMI RX did not return a valid sync_file|Invalid Wayland release fence|Invalid (release|RGA completion) sync_file fence|RGA (imcheck|improcess) failed|RGA did not return a valid release sync_file|Failed to retire RGA job|dup RGA completion fence|VIDIOC_(DQBUF|QBUF|S_FMT)|Error: zwp_linux_buffer_params.create failed|No zwp_linux_explicit_synchronization_v1 global' "$client_log"; then
