...
printf '%s\n' \
    'PASS release-point dynamic-format watchdog-order normal-stop pre-enable-expiry enable-race fired-publication bounded-probe pid-reuse stale-token arm-failure killed-parent killed-before-token deferred-signal bounded-reader'
